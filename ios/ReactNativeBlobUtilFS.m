...
+ (void) setEndOfFile:(NSString *) status {
    if (endOfFile == nil) {
        endOfFile = @"";
    }
    endOfFile = status;
...
+ (void) releaseReadStream {
    [ReactNativeBlobUtilFS setEndOfFile:@""];
}

+ (void) closeReadStream {
    [ReactNativeBlobUtilFS setEndOfFile:@"end"];
}
    ...
    [[self class] getPathFromUri:uri completionHandler:^(NSString *path, ALAssetRepresentation *asset) {
        ...
        @try
        {
            ...
            if(path != nil)
            {
                ...
                [stream open];
                if ([encoding isEqual:@"base64_once"]) {
                    read = [stream read:buffer maxLength:bufferSize];
                    [[self class] emitDataChunks:[NSData dataWithBytes:buffer length:read] encoding:@"base64" streamId:streamId event:event];
                    if(tick > 0)
                    {
                        usleep(backoff);
                    }
                } else if ([encoding  isEqual: @"base64_stream"]) {
                    uint8_t * supplyBuffer;
                    supplyBuffer = (uint8_t *) malloc(3);
                    long bufferSizeToRead = chunkSize;
                    long read = 0;
                    int k;
                    long j;
                    long supplySize = 0;
                    ...
                    while(true) {
                        read = [stream read:buffer maxLength:bufferSizeToRead];
                        if (read <= 0) {
                            NSString * readStatus = [ReactNativeBlobUtilFS getEndOfFile];
                            if(![readStatus isEqual:@"end"]) {
                                if(tick > 0)
                                {
                                    usleep(backoff);
                                }
                                ...
                                if (supplySize != 0) {
                                    // emit the supply data
                                    NSMutableData * supplyData = [NSMutableData dataWithBytes:supplyBuffer length:supplySize];
                                    [[self class] emitDataChunks:supplyData encoding:@"base64" streamId:streamId event:event];
                                }
                                break;
                            }
                        }
                        long newSupplySize = (supplySize + read) % 3;
                        NSMutableData * supplyData = [NSMutableData dataWithBytes:supplyBuffer length:supplySize];
                        NSMutableData * mainData = [NSMutableData dataWithBytes:buffer length:read-newSupplySize];
                        [supplyData appendData:mainData];
                        NSLog(@"supply data is genedrated;");
                        k = 0;
                        for(j = read - newSupplySize; j < read; j++) {
                            supplyBuffer[k] = buffer[j];
                            k++;
                        }
                        supplySize = newSupplySize;
                        bufferSizeToRead = chunkSize - supplySize;
                        NSLog(@"supply data is genedrated %ld and %ld", supplySize, bufferSizeToRead);
                        [[self class] emitDataChunks:supplyData encoding:@"base64" streamId:streamId event:event];
                        if(tick > 0)
                        {
                            usleep(backoff);
